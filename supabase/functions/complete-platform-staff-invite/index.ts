...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (new Date(invite.expires_at) < new Date()) {
      await admin.from('platform_staff_invites').update({ status: 'EXPIRED' }).eq('id', invite.id)
    ...
    if (existing) {
      return new Response(
        JSON.stringify({ error: 'E-mail já cadastrado. Faça login ou peça um novo convite.' }),
